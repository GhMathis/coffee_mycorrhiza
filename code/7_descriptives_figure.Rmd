...
ggplot(map_ug) +
  ...
  main_theme_final -> ug_sampling_map
ug_sampling_map

ggsave(plot = ug_sampling_map, filename = "figures/genus/ug_sampling_map.pdf", units = "cm", width = 12 ,height = 12)
```

```{r}
amf_genus_rare <- read.table("outputs/abundance_tables/Robust_MiSeq_18S_AMf_20241120_125_samples.OTU.filtered.cleaved.nosubstringOTUs.mumu.afm_rarefy_allgenus.txt", header = TRUE, stringsAsFactors = FALSE)
...
genus_tree <- ape::keep.tip(phy = genus_tree, tip = colnames(Y_genus))
```
...
data_18S %>%
  ...
  summarise(kingdom = first(kingdom),
    ...
    mutate(genus = genus %>%
           ...
           str_replace_all("[a-z]__", "Unk. ") %>%
           str_replace("_(\\d+)$", " (\\1 OTU)") %>%
           str_replace("_", " ")) -> df_tip_data_genus
```
...
Y_genus %>% 
  ...
  main_theme_final +
  theme(axis.text.x = element_text(angle = 45)) -> relative_abundance_genus_per_sample
ggsave(plot = relative_abundance_genus_per_sample, filename = "figures/genus/relative_abundance_genus_per_sample.pdf", units = "cm", width = 38 ,height = 24)
```
...
sum_reads_genus_df %>%
  group_by(family) %>%
  summarise(tot_read = sum(sum_rarefied_reads_log))%>%
  ungroup() %>%
  mutate(perc_totread = tot_read/sum(tot_read)) %>%
  arrange(desc(perc_totread))
```
...
Y_genus %>% 
  ...
  rownames_to_column("Sample_code") %>%
  pivot_longer(-Sample_code) %>%
  mutate(name = str_replace(name,"Unknown ", "Unk. "),
  ...
  mutate(family = str_remove(family, "f__"),
         family = str_replace(family, "[*]", "Unk.")) -> sum_reads_genus_df

p_tree_genus <- ggtree(genus_tree, layout = "roundrect") + # %<+% sum_reads_df
  geom_tiplab(size=3, align = T, linesize=.5) +
  theme_tree() +
  xlim_tree(1.2)
sum_reads_genus_df$family
p_tree_genus +
  ...
  scale_fill_manual(values = watlington()[c(3:7,9:13,8)]) +
  theme_classic() +
  theme(
    aspect.ratio = 1,
...
tree_sum_read_plot
ggsave(plot = tree_sum_read_plot, filename = "figures/genus/tree_sum_read_plot.pdf", units = "cm",height = 28, width = 28)
```
