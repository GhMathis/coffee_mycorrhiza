...
save(list_mods_crossvalidation,file =  "outputs/list_mods_crossvalidation_genus.RData")
...
1:length(model_list)%>%
  ...
  left_join( mods_crossvalidation_df %>%
    ...
    summarise(median_abund = median(true_abund, na.rm = T),
              ...
              sd_pred = sd(pred_abund, na.rm = T)), by = join_by(genus_names== genus_names, mod_ID==mod_ID)) %>%
...
recap_pred_df %>%
  ...
  ggplot() +
  geom_point(aes(genus_names, ratio_RMSE_sd, col = as.factor(mod_ID)),cex = 3, alpha =0.8) +
  scale_y_log10() +
...
recap_pred_df %>%
  #filter(!(mod_ID %in% c(1,2,3,4))) %>%
  ggplot() +
  geom_point(aes(genus_names, RMSE, col = as.factor(mod_ID)), cex = 4) +
  scale_y_log10() +
  scale_color_viridis(discrete = T)+
  main_theme
...
mods_crossvalidation_df %>%
 filter(mod_ID == 9, genus_names != "Unknown Gigasporales (6 OTU)") %>%
  ggplot()+
...
mods_crossvalidation_df %>%
 filter(mod_ID %in% c(9,6),genus_names != "Unknown Gigasporales (6 OTU)") %>%
  ggplot()+
...
mods_crossvalidation_df %>%
  filter(mod_ID %in% c(8,9),genus_names != "Unknown Gigasporales (6 OTU)") %>%
...
1:length(model_bin_list) %>%
  purrr::map(\(x) compute.hmsc(model_bin_list[[x]], nfold = 10, nChains = 16,   test.run = F)) -> list_mods_bin_crossvalidation

save(list_mods_bin_crossvalidation, file = "outputs/list_mods_bin_crossvalidation_genus.RData")
...
1:ncol(list_mods_bin_crossvalidation[[1]]$m_converg$Y) %>%
  map(\(x) compute.ROC(observed =  list_mods_bin_crossvalidation[[7]]$m_converg$Y[,x], predicted = list_mods_bin_crossvalidation[[7]]$mPredY[,x],
            pred_name = colnames(list_mods_bin_crossvalidation[[7]]$m_converg$Y)[x])) %>%
    bind_rows() -> ROC_df
ROC_df %>%
  ggplot()+
...
ROC_df %>%
  ...
  select(pred_name, threshold, balanced_score) %>%
  distinct() %>%
...
absence_mat <- sweep(list_mods_bin_crossvalidation[[7]]$mPredY, 2,
                       balanced_score_df$threshold, 
                       FUN = function(x, th) x > th)


mPredY_hurdel <- list_mods_crossvalidation[[9]]$mPredY
...
mPredY_hurdel %>%
    as.data.frame() %>%
    rownames_to_column("Sample_code") %>%
    pivot_longer(-Sample_code, names_to = "genus_names", values_to = "pred_abund") -> hurdel_long

log1p(Y_genus) %>%
  as.data.frame() %>%
  rownames_to_column("Sample_code") %>%
  pivot_longer(-Sample_code, names_to = "genus_names", values_to = "true_abund") -> true_value_long

hurdel_long %>%
  left_join(true_value_long, by = join_by(genus_names,Sample_code)) -> abund_test_df
...
abund_test_df %>%
  ...
  summarise(n = n(),
            ...
            mod_ID = "hurdel_9_7") -> recap_hurdel_df


recap_pred_df %>%
  group_by(mod_ID) %>%
  summarise(mean(RMSE))
mean(recap_hurdel_df$RMSE)
...
abund_test_df %>%
  mutate(pred_abund = exp(pred_abund)-1,
         true_abund = exp(true_abund)-1) %>%
  filter( genus_names != "Unknown Gigasporales (6 OTU)")%>%
  ggplot()+
  geom_point(aes(true_abund,pred_abund, col = log(pred_abund/(true_abund+1))), cex = 4) +
  facet_wrap(~genus_names)+
    #facet_grid(col = vars(genus_names), row = vars(mod_ID), scale  = "free") +
...
abund_test_df %>%
  mutate(pred_abund = exp(pred_abund)-1,
         true_abund = exp(true_abund)-1) %>%
  filter( !(genus_names %in% c("Unknown Gigasporales (6 OTU)","Glomeraceae (452 OTU)", "Acaulosporaceae (88 OTU)")))%>%
  ggplot()+
  geom_point(aes(true_abund,pred_abund, col = log(pred_abund/(true_abund+1))), cex = 4) +
...
```{r, purl= FALSE}
mods_crossvalidation_df %>%
  filter(mod_ID == 9, !(genus_names %in% c("Unknown Gigasporales (6 OTU)","Glomeraceae (452 OTU)", "Acaulosporaceae (88 OTU)")))%>%
  ggplot()+
...
```{r, purl= FALSE}
recap_pred_df %>%
 filter(mod_ID == 9,ratio_RMSE_sd<1) %>%
  pull(genus_names) -> good_prediction
data_spatial_df %>%
