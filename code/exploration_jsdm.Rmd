...
postBeta2$mean %>%
  as.data.frame() %>%
  mutate(coffee_system = c("intercept", as.character(unique(soil_df2$coffee_system)[2:5]))) %>%
  pivot_longer(-coffee_system, values_to = "beta_support", names_to = "genus") -> estimated_beta_support2

estimated_beta_support2 %>%
  ggplot()+
  geom_tile(aes(coffee_system, genus, fill =beta_support)) +
  scale_fill_gradient2(
    low = "blue",
    mid = "white",
    high = "red",
  ...
  theme_minimal()
plotBeta(m_converg2, post = postBeta2, param="Support", supportLevel = 0.95)
```

```{r}

VP2 = computeVariancePartitioning(m_converg2, groupnames = c("coffee_system")) 
VP2$vals %>%
  as.data.frame %>%
  rownames_to_column("var") %>%
  pivot_longer(-var) %>%
...
```{r}
Gradient2 = constructGradient(m_converg2, focalVariable ="coffee_system")
...
plotGradient(m_converg2, Gradient2, pred=predY2, measure="Y",index = 2, showData = TRUE, jigger = 0.2)
...
binary_sbm$memberships$col == 8->id_out
temp2 <-temp[,!id_out]
temp2%>%
  log1p()%>%
  plotMyMatrix()
hist(temp2)
estimateBipartiteSBM(temp2, model = "poisson") -> poisson_sbm
```
