...
recap_pred_df %>%
  group_by(mod_ID) %>%
  summarise(RMSE_mean = median(RMSE)) %>%
  arrange(RMSE_mean)
```
...
mods_crossvalidation_df %>%
  group_by(mod_ID,Sample_code) %>%
  summarise(sum(exp(true_abund)))
mods_crossvalidation_df %>%
  group_by(mod_ID,Sample_code) %>%
  summarise(round(sum(exp(pred_abund))))
```
```{r}
mods_crossvalidation_df %>%
...
mods_crossvalidation_df %>%
  filter(pred_abund >10)
exp(10)
mods_crossvalidation_df %>%
  # remove one extreme outier prediced value
  filter(pred_abund <10^20) %>%
  filter(mod_ID == "mod_pois_full") %>%
  ...
  theme(legend.position = "none",
        strip.text = element_text(margin = margin(0.05,0,0.05,0, "cm"),size = 5))-> vaildation_mod_abund_genus_poly

vaildation_mod_abund_genus_poly

ggsave(plot = vaildation_mod_abund_genus_poly, filename = "figures/vaildation_mod_abund_genus_poly.pdf", units = "cm",
        height = 16, width = 16)
```
...
# ------------------------------------------------------------------
models <- list(abund = list_mods_crossvalidation[[8]]$m_converg, bin = list_mods_bin_crossvalidation[[8]]$m_converg) # , 
...
# ggsave(plot = map_occurence_best_predicted_genus, filename = "figures/map_occurence_best_predicted_genus.svg", units = "cm", width = 33 ,height = 20 )
```
...
data_spatial_df %>%
    ...
    pivot_longer(-c(x, y, ndvi, precip, tmax, tdelta, wind)) %>%
    filter(!(name %in% c("Glomeraceae_gen11 (7 OTU)",
                       "Glomeraceae_gen05 (4 OTU)",
                       "Funneliformis (2 OTU)",
                       "Unknown Glomeraceae (120 OTU)",
                       "Acaulospora (88 OTU)"))) %>%
    mutate(value = value) %>%
    ggplot() +
    facet_wrap(~name, scale = "free") +
    geom_raster(aes(x, y, fill = log1p(value))) +
    scale_fill_viridis(option = "magma") +
...
data_spatial_df %>%
    bind_cols(EpredY_map) %>%
    pivot_longer(-c(x, y, ndvi, precip, tmax, tdelta, wind)) %>%
    filter(name %in% c("Acaulospora (88 OTU)")) %>%
    mutate(value = value) %>%
    ggplot() +
    #facet_wrap(~name, scale = "free") +
    geom_raster(aes(x, y, fill = value)) +
...
data_spatial_df %>%
    bind_cols(EpredY_map) %>%
    pivot_longer(-c(x, y, ndvi, precip, tmax, tdelta, wind)) %>%
    filter(name %in% c("Gigaspora (14 OTU)")) %>%
    mutate(value = value) %>%
    ggplot() +
    #facet_wrap(~name, scale = "free") +
    geom_raster(aes(x, y, fill = value)) +
...
data_spatial_df %>%
    bind_cols(EpredY_map) %>%
    pivot_longer(-c(x, y, ndvi, precip, tmax, tdelta, wind)) %>%
    filter(name %in% c("Septoglomus (16 OTU)")) %>%
    mutate(value = value) %>%
    ggplot() +
    #facet_wrap(~name, scale = "free") +
    geom_raster(aes(x, y, fill = value)) +
