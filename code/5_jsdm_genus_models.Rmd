...
mod_precip_poly <- Hmsc(
  Y = Y_genus,
  XData = covariate_standa_df,
  XFormula = ~ poly(precip,2),
  # phyloTree = genus_tree,
  studyDesign = studyDesign,
  ranLevels = list(sample = rL),
  distr = "poisson"
...
```{r}
model_list %>%
  purrr::map(\(x) sampleMcmc(
    x,
    thin = thin, samples = samples, transient = transient,
    nChains = nChains, verbose = verbose, nParallel = nChains)) -> list_mods
computeWAIC(list_mods[[1]]$m_converg$)
WAIC = unlist(lapply(list_mods, FUN = computeWAIC))
WAIC
