...
colnames(Y_genus_trucated)
genus_tree_filter$tip.label

library(GGally)
covariate_df %>% 
  dplyr::select(ndvi, precip) %>%
  ggpairs()
...
m_converg2_bin <- sampleMcmc(
  m_bin,
  thin = thin, samples = samples, transient = transient,
  nChains = nChains, verbose = verbose, nParallel = 12
)
save(m_converg2_bin, file = "outputs/hmsc_genus_bin_ndvi.Rdata")
load(file = "outputs/hmsc_genus_bin_ndvi.Rdata")
```
...
beta_results <- postBeta2$mean
rownames(beta_results) <- colnames(m_converg2$X)
beta_results <- beta_results * ((postBeta2$support > 0.9) + (postBeta2$support < (1 - 0.9)) > 0)
beta_results <- t(beta_results)[genus_tree_filter$tip.label, colnames(m_converg2$X)]
...
rownames(beta_bin_results) <- colnames(m_converg2_bin$X)
beta_bin_results <- beta_bin_results * ((postBeta_bin2$support > 0.9) + (postBeta_bin2$support < (1 - 0.9)) > 0)
beta_bin_results <- t(beta_bin_results)[genus_tree_filter$tip.label, colnames(m_converg2_bin$X)]
...
VP2 <- computeVariancePartitioning(m_converg2, na.ignore = TRUE)
...
VP_bin2 <- computeVariancePartitioning(m_converg2_bin, na.ignore = TRUE)
VP_bin2$vals %>%
  as.data.frame() %>%
  rownames_to_column("var") %>%
  pivot_longer(-var) %>%
...
corrplot(toPlot, method = "color", col = colorRampPalette(c("blue", "white", "red"))(200),
         ...
         title = paste("random effect level:", m_converg2$rLNames[1]),
