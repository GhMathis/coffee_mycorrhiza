...
```{r}
elevation_rast <- raster("data/raster/wc2.1_2.5m_elev/wc2.1_2.5m_elev.tif")
...
if(file.exists("outputs/raster/wind_2023.tif")){
  ...
  elvation_resampled <- raster::resample(elevation_rast, land_degradation_lower_res, method = "ngb")
  altitude_uganda <- mask(elvation_resampled, land_degradation_lower_res)
  writeRaster(altitude_uganda,"outputs/raster/altitude_uganda.tif",options=c('TFW=YES'), overwrite=TRUE)
  }

plot(altitude_uganda)

for(name in c("pH.tif", "clay.tif", "nitrogen.tif", "organic_carbone.tif", "sand.tif", "silt.tif")){
  temp_rast <- raster(file.path("data/raster/soil_data/",name))
  temp_resampled <- raster::resample(temp_rast, land_degradation_lower_res, method = "ngb")
  temp_resampled <- mask(temp_resampled, land_degradation_lower_res)
  writeRaster(temp_resampled,file.path("outputs/raster/",name),options=c('TFW=YES'), overwrite=TRUE)
}
```
