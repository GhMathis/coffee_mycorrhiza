...
wgs84_coords %>%
  ...
  left_join(land_degradation_df, by = "Sample_code") -> wgs84_coords
...
m2 = Hmsc(Y = Y_family_trucated, XData = metadata_filter, 
          XFormula = ~Land_Cover + land_degradation, 
...
land_degradation_raster_df <- rasterToPoints(land_degradation_lower_res) %>%
  as.data.frame() %>%
  mutate(Uganda_ldma_2020 = factor(Uganda_ldma_2020,labels = levels(land_degradation_lower_res)[[1]]$land_degradation)) %>%
  rename(land_degradation = "Uganda_ldma_2020")
land_cover_type
land_cover_raster_df <- rasterToPoints(land_cover_resampled) %>%
  as.data.frame() %>%
  filter(Land_Cover != 0) %>%
  left_join(land_cover_type, by = join_by(Land_Cover == ID)) %>% 
  dplyr::select(-Land_Cover) %>%
  rename(Land_Cover = "Land_Cover.y")
...
# xy.grid = as.matrix(cbind(test_df$x, test_df$y)) 
# XData.grid = data.frame(land_degradation = as.factor(test_df$land_degradation),
#                         Land_Cover = as.factor(test_df$Land_Cover)) 
...
# 
# # lapply(predY, function (x) as.integer(x)) -> predYmin
# 
# 
# EpredY = predY[[1]]
# EpredY2 = predY2[[1]]
