...
# 
# 
# save(list_mods_crossvalidation,file =  "outputs/list_mods_crossvalidation_family_pois_log.RData")
load(file = "outputs/list_mods_crossvalidation_family_pois_log.RData")
```
...
```{r, purl = FALSE}
mods_crossvalidation_df %>%
   filter(mod_ID == "mod_pois_full") %>%
  group_by(family_names) %>%
  summarise(mean_abund = mean(true_abund)) %>%
  arrange(mean_abund) %>% pull(family_names) -> order_family_by_abund

mods_crossvalidation_df %>%
  filter(mod_ID == "mod_pois_full") %>%
  mutate(family_names = factor(family_names, levels =  order_family_by_abund)) %>%
  ggplot()+
  geom_point(aes(true_abund, pred_abund, fill = family_names),alpha =0.8, shape= 21, cex = 4) +
  facet_wrap(~family_names)+
  geom_abline(slope=1) +
  labs(x= "Observed abundances", y = "Predicted abudances", fill = "Taxa/Family names")+
  scale_fill_viridis(discrete = T) +
  main_theme_final+
  theme(legend.position = "none",
        strip.text = element_text(margin = margin(0.05,0,0.05,0, "cm"),size = 5))-> vaildation_mod_abund_family_full

vaildation_mod_abund_family_full

ggsave(plot = vaildation_mod_abund_family_full, filename = "figures/vaildation_mod_abund_family_full.pdf", units = "cm",
        height = 16, width = 16)
```
...
# 
# save(list_mods_bin_crossvalidation, file = "outputs/list_mods_bin_crossvalidation_family2.RData")
load(file = "outputs/list_mods_bin_crossvalidation_family2.RData")
```
...
ROC_df %>% 
  ...
  main_theme_final -> ROC_curve_mod_bin_family_full

ggsave(plot = ROC_curve_mod_bin_family_full, filename = "figures/AUC_curve_mod_bin_family_full.pdf", units = "cm",
        height = 16, width = 16)

ROC_curve_mod_bin_family_full
```
...
p <- ggtree(family_tree_filter, layout = "roundrect") +
  ...
  theme_tree() 
colnames(beta_results) = c("Intercept", "Biomass quantity index", "Precipitation", "T° max", "T° delta", "Wind")
...
gheatmap(p, beta_results, offset = 1, width = 1, font.size = 5,
  ...
  main_theme_final+
  theme_void() -> effect_plot_family_abund

effect_plot_family_abund

ggsave(plot = effect_plot_family_abund, filename = "figures/effect_plot_family_abund.pdf", units = "cm",
        height = 18, width = 20)
```
...
gheatmap(p, beta_results_bin, offset = 1, width = 1, font.size = 5,
  ...
  main_theme_final+
  theme_void() -> effect_plot_family_bin

ggsave(plot = effect_plot_family_bin, filename = "figures/effect_plot_family_bin.pdf", units = "cm",
        height = 18, width = 20)
```
...
lambda_df %>%
  ...
  main_theme_final -> phylogentic_signal_family_models

ggsave(plot = phylogentic_signal_family_models, filename = "figures/phylogentic_signal_family_models.pdf", units = "cm",
        height = 16, width = 16)
phylogentic_signal_family_models
```
...
VP2$vals %>%
  ...
  labs(x= "Family names", y = "Variance", fill = "Covariates")+
  scale_fill_viridis(discrete = T)+
  main_theme_final +
  theme(axis.text.x = element_text(angle = 45)) -> variance_partitioning_abund_family
variance_partitioning_abund_family
ggsave(plot = variance_partitioning_abund_family, filename = "figures/variance_partitioning_abund_family.pdf", units = "cm",
        height = 16, width = 16)
...
ggsave(plot = variance_partionning_occurence, filename = "figures/variance_partionning_occurence.pdf", units = "cm",
        height = 16, width = 16)
...
```{r}
recap_pred_bin_df %>%
  filter(mod_ID == "mod_bin_full", AUC >0.75| is.na(AUC)) %>% #, family_names!="Diversisporaceae (36 OTU)"
...
ggsave(plot = map_occurence_allfamily, filename = "figures/map_occurence_allfamily.pdf", units = "cm", width = 33 ,height = 20)
ggsave(plot = map_occurence_best_predicted_family, filename = "figures/map_occurence_best_predicted_family.pdf", units = "cm", width = 33 ,height = 20 )
ggsave(plot = map_richness, filename = "figures/map_richness.pdf", units = "cm", width = 18 ,height = 18 )
```
...
ggsave(plot = environemental_maps, filename = "figures/environemental_maps.pdf", units = "cm", width = 33 ,height = 20)
environemental_maps
```
...
data_spatial_df %>%
    ...
    pivot_longer(-c(x, y, ndvi, precip, tmax, tdelta, wind)) %>%
    filter(name == "Glomeraceae (452 OTU)") %>%
    mutate(value = value) %>%
    ggplot() +
    facet_wrap(~name) +
    geom_raster(aes(x, y, fill = value)) +
...
ggsave(plot = abundance_Glomeraceae_map, filename = "figures/abundance_Glomeraceae_map.pdf", units = "cm", width = 18 ,height = 18 )
...
data_spatial_df %>%
    bind_cols(EpredY_map) %>%
    pivot_longer(-c(x, y, ndvi, precip, tmax, tdelta, wind)) %>%
    filter(name == "Acaulosporaceae (88 OTU)") %>%
    mutate(value = value) %>%
    ggplot() +
    facet_wrap(~name) +
    geom_raster(aes(x, y, fill = value)) +
...
ggsave(plot = abundance_Acaulosporaceae_map, filename = "figures/abundance_Acaulosporaceae_map.pdf", units = "cm", width = 18 ,height = 18 )
...
data_spatial_df %>%
    ...
    scale_fill_viridis(option = "magma") +
    labs(x = "Longitude", y = "Latitude", fill = "Abundance (log)") +
    main_theme_final -> medium_abundante_family_map

ggsave(plot = medium_abundante_family_map, filename = "figures/medium_abundante_family_map.pdf", units = "cm", width = 33 ,height = 20)
medium_abundante_family_map
```
```{r, purl = FALSE}
data_spatial_df %>%
    bind_cols(EpredY_map) %>%
    ...
    scale_fill_viridis(option = "magma") +
    labs(x = "Longitude", y = "Latitude", fill = "Abundance (log)") +
    main_theme_final -> low_abundante_family_map

ggsave(plot = low_abundante_family_map, filename = "figures/low_abundante_family_map.pdf", units = "cm", width = 33 ,height = 20)
low_abundante_family_map
```
