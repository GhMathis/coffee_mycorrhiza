...
mod_pois <- Hmsc(
  ...
  XFormula = ~ndvi + precip + tmin + tdelta + wind,
...
library(purrr)
1:6 %>%
  purrr::map(\(x) compute.hmsc(mod_list[[x]], nfold = 10, nChains = 14,   test.run = F)) -> list_mods_crossvalidation
...
list_mods_crossvalidation[[1]]$hmsc_mod$XData
list_mods_crossvalidation[[2]]$hmsc_mod$XData
1:6 %>%
...
list_mods_crossvalidation[[1]][["MF_expl"]]
...
```{r include=FALSE}
list_mods_crossvalidation[[3]]$m_converg -> m_converg_selected
