...
length(covariate_df$Sample_code) -> n_samples

sample(1:n_samples, round(n_samples*0.8)) -> sample_selection

covariate_df$Sample_code[sample_selection] -> training_samples 
covariate_df$Sample_code[-sample_selection] -> testing_samples 
...
Y_family_trucated %>%
  ...
  column_to_rownames("soil_code")  -> amf_family_rare_training

Y_family %>%
  as.data.frame() %>%
  rownames_to_column("soil_code") %>%
  filter(soil_code %in% testing_samples) %>%
  column_to_rownames("soil_code") -> amf_family_rare_testing

colSums(amf_family_rare_training, na.rm = T)

colSums(amf_family_rare_testing, na.rm = T)
```
...
# Sample names from Y
family_matrix_samples <- rownames(amf_family_rare_training)

amf_family_rare_training %>%
  as.data.frame() %>%
  rownames_to_column("soil_code") %>%
  pivot_longer(-soil_code) %>%
  ggplot() +
...
amf_family_rare_testing %>%
  ...
  facet_wrap(~name, scales = "free") +
  geom_histogram(aes(value)) +
  labs(x = "abundances without zeros (testing set)")

studyDesign <- data.frame(sample = rownames(amf_family_rare_training), stringsAsFactors = TRUE)
...
Y_family_bin_training <- amf_family_rare_training %>%
  as.data.frame() %>%
  rownames_to_column("soil_code") %>%
  pivot_longer(-soil_code) %>%
...
m_converg_bin_training <- sampleMcmc(
  m_bin,
  thin = thin, samples = samples, transient = transient,
  nChains = nChains, verbose = verbose, nParallel = 12
)
...
```{r include=FALSE}
postBeta2 <- getPostEstimate(m_converg_training, parName = "Beta")
beta_results <- postBeta2$mean
mpost2 <- convertToCodaObject(m_converg_training)
gelman.diag(mpost2$Beta, multivariate = FALSE) -> gelman_diag_df2
gelman_diag_df2$psrf

postBeta_bin2 <- getPostEstimate(m_converg_bin_training, parName = "Beta")
mpost_bin2 <- convertToCodaObject(m_converg_bin_training)
gelman.diag(mpost_bin2$Beta, multivariate = FALSE) -> gelman_diag_bin_df2
gelman_diag_bin_df2$psrf

beta_results <- postBeta2$mean
rownames(beta_results) <- colnames(m_converg_training$X)
...
predY <- stats::predict(m_converg_training, Gradient = Gradient)

Gradient2 <- prepareGradient(m_bin, XDataNew = covariate_testing)
predY2 <- stats::predict(m_converg_bin_training, Gradient = Gradient2)
...
amf_family_rare_testing %>%
  as.data.frame() %>%
  rownames_to_column("soil_code") %>%
  pivot_longer(-soil_code, names_to = "family_names", values_to = "true_occurence") %>%
  mutate(true_occurence = ifelse(true_occurence==0, 0,1)) -> test_samples_bin_long

threshold = 0.5
for (threshold in seq(0.01, 1, length.out = 100)){
  temp_pred <- EpredY2
  ...
  temp_pred %>%
    as.data.frame() %>%
    rownames_to_column("soil_code") %>%
    pivot_longer(-soil_code, names_to = "family_names", values_to = "pred_abund")%>%
    left_join(test_samples_bin_long, by = join_by(family_names,soil_code)) -> temp_pred_long
...
```{r}
EpredY_hurdel <- EpredY
EpredY_hurdel[EpredY2 <0.8] <- 0
```
