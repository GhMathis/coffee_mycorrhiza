...
```{r}
raster.standa <- function(r){
  r_mean <- cellStats(r, "mean")
  r_sd   <- cellStats(r, "sd")
  r_z <- (r - r_mean) / r_sd
  return(r_z)
  }
...
```{r}
delta_t_uganda <- raster("outputs/raster/tmax_2023_lower_res.tif") - raster("outputs/raster/tmin_2023_lower_res.tif")
delta_t_uganda <- delta_t_uganda %>%
  raster.standa
...
ape::write.tree(genus_tree_filter, "outputs/data_for_models/genus_tree.tree")
```
